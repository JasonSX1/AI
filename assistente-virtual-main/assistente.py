...
def iniciar_atuadores():
    ...
    if iniciar_lampada():
        atuadores.append({"nome": "lâmpada",
                         "atuacao": atuar_sobre_lampada})
    if iniciar_som():
        atuadores.append({"nome": "Sistema de som",
...
def ativar_linha_de_comando():
    while True:
        ...
        if gravado:
            ...
            atuar(acao, dispositivo, atuadores)    
...
def reconhecer_comando():
    if "audio" not in request.files:
        return Response(status=400, response="arquivo de áudio não encontrado")
